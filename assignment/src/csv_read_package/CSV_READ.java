...
public class CSV_READ {
	 ...
	 */
	public static LinkListGenric<Credits> readCsvBufferReaderCredits(String fileName) {
		LinkListGenric<Credits> list = new LinkListGenric<Credits>(); //linklist of credists to store data
	 ...
	 * @return linklist<Credits>
	 */
	private static LinkListGenric<Cast> parseCast(JSONArray array) {	
		LinkListGenric<Cast> list = new LinkListGenric<Cast>();
			for(int i = 0; i < array.length(); i++) {//iterate over the values of json array
				try {
					...
					list.addToStart(new Cast(obj.getInt("id"), obj.getString("name"),obj.getInt("cast_id"), obj.getString("character"), obj.getString("credit_id"), (obj.getInt("gender") == 1)?"male":"female", obj.getInt("order")));
				}catch(Exception ex) {
					//System.err.println(ex);
				}
			}
	 ...
	 */
	private static LinkListGenric<Incharge> parseIncharge(JSONArray array) {	
		LinkListGenric<Incharge> list = new LinkListGenric<Incharge>();
			for(int i = 0; i < array.length(); i++) {//iterate over items of array
				try {
					...
					list.addToStart(new Incharge(obj.getInt("id"), obj.getString("name"), obj.getString("department"), (obj.getInt("gender") == 1)?"male":"female", obj.getString("job"), obj.getString("credit_id")));
				}catch(Exception ex) {
					//System.err.println(ex);
				}
			}
...
////////////////////////Reading Movie csv ////////////////////////
	 ...
	 */
	public static BinaryTreeMovies readCsvBufferReaderMovie(String fileName) {
		BinaryTreeMovies bTreeMovies = new BinaryTreeMovies();//initalize binary tree
...
//		        	}
	 ...
	 * @return
	 */
	private static LinkListGenric<KeyWord> parseMovieKeyWords(String str) {
		try {
			...
			LinkListGenric<KeyWord> list = new LinkListGenric<KeyWord>();
			for(int i = 0; i < jsonArray.length(); i++) {
				try {
					JSONObject obj =  jsonArray.getJSONObject(i);//extract data and store it to its crossponding list
					list.addToStart(new KeyWord(obj.getInt("id"), obj.getString("name")));
				}catch(Exception ex) {
					System.err.println(ex);
				}
			}
	 ...
	 * @return
	 */
	private static LinkListGenric<ProductionCompanies> parseProdCompanies(String str) {
		try {
			str = str.replace("\"\"", "\""); //replace "" "
			if(str.charAt(0) == '\"') {
				str = str.substring(1, str.length());//take substring to remove " from start
			...
			JSONArray jsonArray = new JSONArray(str); //convert string to json array
			LinkListGenric<ProductionCompanies> list = new LinkListGenric<ProductionCompanies>();
			for(int i = 0; i < jsonArray.length(); i++) {
				try {
					JSONObject obj =  jsonArray.getJSONObject(i);//extract data and store it to its crossponding list
					list.addToStart(new ProductionCompanies(obj.getInt("id"), obj.getString("name")));
				}catch(Exception ex) {
					System.err.println(ex);
				}
			}
	 ...
	 * @return
	 */
	private static LinkListGenric<ProductionCountries> parseProductionCountries(String str) {
		try {
			str = str.replace("\"\"", "\""); //replace "" "
			if(str.charAt(0) == '\"') {
				str = str.substring(1, str.length());//take substring to remove " from start
			...
			JSONArray jsonArray = new JSONArray(str); //convert string to json array
			LinkListGenric<ProductionCountries> list = new LinkListGenric<ProductionCountries>();
			for(int i = 0; i < jsonArray.length(); i++) {
				try {
					JSONObject obj =  jsonArray.getJSONObject(i);//extract data and store it to its crossponding list
					list.addToStart(new ProductionCountries(obj.getString("iso_3166_1"), obj.getString("name")));
				}catch(Exception ex) {
					System.err.println(ex);
				}
			}
	 ...
	 * @return
	 */
	private static LinkListGenric<SpokenLanguages> parseSpokenLanguages(String str) {
		try {
			str = str.replace("\"\"", "\""); //replace "" "
			if(str.charAt(0) == '\"') {
				str = str.substring(1, str.length());//take substring to remove " from start
			...
			JSONArray jsonArray = new JSONArray(str); //convert string to json array
			LinkListGenric<SpokenLanguages> list = new LinkListGenric<SpokenLanguages>();
			for(int i = 0; i < jsonArray.length(); i++) {//iterate over json array 
				try {
					...
					list.addToStart(new SpokenLanguages(obj.getString("iso_639_1"), obj.getString("name")));
				}catch(Exception ex) {
					System.err.println(ex);
				}
			}
			return (list.length() > 0)?list: null;//return list if length is greater then 0
		} catch(Exception ex) {
			System.err.println(ex);
			System.exit(1);
			return null;
	 ...
	 * @return
	 */
	private static LinkListGenric<Genres> parseGenres(String str) {
		try {
			...
			LinkListGenric<Genres> list = new LinkListGenric<Genres>();  //define linklist of genres
			...
			for(int i = 0; i < jsonArray.length(); i++) {
				try {
					...
					list.addToStart(new Genres(obj.getInt("id"), obj.getString("name")));
				}catch(Exception ex) {
					System.err.println(ex);
				}
			}
			return (list.length() > 0)?list: null;// return genera list if it is not null
		} catch(Exception ex) {
			System.err.println(ex);
			System.exit(1);
			return null;
